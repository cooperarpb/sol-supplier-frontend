...
.addresses-fields
  input(
    type="hidden",
    :name="prefix + '[address_attributes][id]'",
    :value="address.id",
    v-if="address"
  )
  ...
  .row
    ...
    .two.columns
      select-field#state(
        ...
        require,
        :error="errors[errorPrefix + 'address.state']"
      )

    .three.columns.input-field(
      :class="this.errors && this.errors[this.errorPrefix + 'address.city'] && this.address.city_id == null ? 'errors' : ''"
    )
      label
        | {{ $t('.labels.city') }}
      .error.mb-1(
        v-if="this.errors && this.errors[this.errorPrefix + 'address.city'] && this.address.city_id == null"
      )
